...
def exact_sum?(k, coins, res=[], bad=[])
#   #write your code here
  return true if coins.reduce(&:+).eql? k
  unless res.include? true
    coins.combination(coins.size - 1).to_a.each do |sub_coins|
      next if bad.include? sub_coins.sort
      res << true << k if sub_coins.reduce(&:+).eql? k
      bad << sub_coins.sort
      exact_sum?(k, sub_coins, res, bad) if sub_coins.size > 2
    end
  end
